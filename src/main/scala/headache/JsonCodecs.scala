...
//  }
  ...
  implicit val gatewayEventGuildFormat: Format[GatewayEvents.Guild] = new Format[GatewayEvents.Guild] {
    ...
    val ownerId = (__ \ "ownerId").format[Snowflake]
    val region = (__ \ "region").format[String]
    val afkChannelId = (__ \ "afkChannelId").formatNullable[Snowflake]
    val afkTimeout = (__ \ "afkTimeout").format[Int]
    val embedEnabled = (__ \ "embedEnabled").formatNullable[Boolean]
    val embedChannelId = (__ \ "embedChannelId").formatNullable[Snowflake]
    val verificationLevel = (__ \ "verificationLevel").format[Int]
    val defaultMessageNotifications = (__ \ "defaultMessageNotifications").format[NotificationLevel]
    val explicitContentFilter = (__ \ "explicitContentFilter").format[ExplicitContentFilterLevel]
    val roles = (__ \ "roles").formatWithDefault[Seq[Role]](Seq.empty)
    val emojis = (__ \ "emojis").formatWithDefault[Seq[Emoji]](Seq.empty)
    val features = (__ \ "features").formatWithDefault[Seq[String]](Seq.empty)
    val mfaLevel = (__ \ "mfaLevel").format[Int]
    val applicationId = (__ \ "applicationId").formatNullable[Snowflake]
    val widgetEnabled = (__ \ "widgetEnabled").formatNullable[Boolean]
    val widgetChannelId = (__ \ "widgetChannelId").formatNullable[Snowflake]
    val joinedAt = (__ \ "joinedAt").formatNullable[Instant]
    val large = (__ \ "large").formatWithDefault[Boolean](false)
    val unavailable = (__ \ "unavailable").formatWithDefault[Boolean](false)
    val memberCount = (__ \ "memberCount").format[Int]
    val members = (__ \ "members").formatWithDefault[Seq[GuildMember]](Seq.empty)
    val voiceStates = (__ \ "voiceStates").formatWithDefault[Seq[VoiceState]](Seq.empty)
    val channels = (__ \ "channels").formatWithDefault[Seq[Channel]](Seq.empty)
    val presences = (__ \ "presences").formatWithDefault[Seq[GuildPresence]](Seq.empty)
  ...
  implicit val readyFormat: Format[GatewayEvents.Ready] = (
    ...
    (__ \ "privateChannels").format[Seq[Channel]] and
    (__ \ "guilds").format[Seq[UnavailableGuild Either GatewayEvents.Guild]] and
    (__ \ "userSettings").formatWithDefault[Option[UserSettings]](None) and
    (__ \ "readState").format[Seq[GatewayEvents.ReadState]] and
    (__ \ "_trace").format[Seq[String]]
